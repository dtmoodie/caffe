...
__host__ __device__ float BBoxSizeGPU(const float* bbox,
    const bool normalized);
template CAFFE_EXPORT 
__host__ __device__ double BBoxSizeGPU(const double* bbox,
    const bool normalized);
...
template CAFFE_EXPORT __host__ __device__ 
float JaccardOverlapGPU(const float* bbox1,
    const float* bbox2);
template CAFFE_EXPORT __host__ __device__ 
double JaccardOverlapGPU(const double* bbox1,
    const double* bbox2);
...
template CAFFE_EXPORT __device__
void ClipBBoxGPU(const float* bbox, float* clip_bbox);
template CAFFE_EXPORT __device__
void ClipBBoxGPU(const double* bbox, double* clip_bbox);
...
template CAFFE_EXPORT void DecodeBBoxesGPU(const int nthreads,
...
template CAFFE_EXPORT void PermuteDataGPU(const int nthreads,
...
template CAFFE_EXPORT void ComputeOverlappedGPU(const int nthreads,
...
template CAFFE_EXPORT void ComputeOverlappedByIdxGPU(const int nthreads,
...
template CAFFE_EXPORT void GetDetectionsGPU(const float* bbox_data,
          const float* conf_data,
          const int image_id, const int label, const vector<int>& indices,
          const bool clip_bbox, Blob<float>* detection_blob);
template CAFFE_EXPORT void GetDetectionsGPU(const double* bbox_data,
          const double* conf_data,
          const int image_id, const int label, const vector<int>& indices,
